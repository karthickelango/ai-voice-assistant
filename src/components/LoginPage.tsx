...
export const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        const validatedUser =
          data.user?.username || trimmedUser;

        // Proceed immediately to asking for
        // Microphone and Location access
      ...
      if (
        trimmedUser === 'Assistant_AI' &&
        trimmedPass === 'Thruv@2023'
      ) {
        await executePermissionPrompts(
          'Assistant_AI'
        );
      } else {
        setErrorMessage(
